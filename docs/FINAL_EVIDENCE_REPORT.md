...
- `npm run test:concurrency`: passed (1 Firestore emulator test file, 2 tests). Expected duplicate-document warnings appeared for intentionally raced same-user writes.
...
- A focused Firestore emulator concurrency gate races same-user support and verification writes and verifies only one persisted action/count wins.
...
- Transaction/concurrency behavior has focused parallel emulator coverage for same-user support and verification writes; a full API-level race matrix remains future hardening.
...
- `npm run test:concurrency`: passed (1 file, 2 tests) against the Firestore emulator.
