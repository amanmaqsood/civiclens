...
function actorMeta(actorType: CivicEvent["actorType"]) {
  if (actorType === "citizen") return { label: "Citizen", icon: User, className: "text-status-resolved-ink bg-verify/10 border-verify/20" };
  ...
  if (actorType === "worker") return { label: "Worker", icon: Cog, className: "text-ink-2 bg-slate/10 border-slate/20" };
...
function statusMeta(event: CivicEvent) {
  if (event.status === "failed" || event.severity === "error") {
    return { label: "Failed", icon: XCircle, className: "text-status-overdue-ink bg-alert/10 border-alert/20" };
  }
  ...
  return { label: "Recorded", icon: CheckCircle2, className: "text-status-resolved-ink bg-verify/10 border-verify/20" };
}
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 border-b border-hairline pb-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 id="case-accountability-ledger-title" className="text-lg font-display font-black text-ink flex items-center gap-2">
            <Lock className="h-4 w-4 text-status-resolved-ink" />
            Accountability ledger
          </h3>
          <p className="mt-1 text-sm font-semibold leading-relaxed text-ink-2">
            Server-owned append-only events for this case.
          </p>
        </div>
        <span className="inline-flex w-fit items-center gap-1.5 rounded-xl border border-hairline bg-paper px-2.5 py-1.5 text-sm font-mono font-semibold text-ink-2">
      ...
      {loading ? (
        ...
        <p role="alert" className="rounded-xl border border-alert/20 bg-alert/10 p-3 text-sm font-semibold text-status-overdue-ink">
          {error}
        </p>
      ) : visibleEvents.length === 0 ? (
        <p className="rounded-xl border border-hairline bg-paper p-3 text-sm font-semibold text-ink-2">
        ...
        <>
              ...
              return (
                <li key={event.id} className="rounded-xl border border-hairline bg-paper p-3">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex flex-col gap-1 text-left text-sm font-mono text-ink-2 sm:text-right">
          ...
          <div className="overflow-x-auto rounded-xl border border-hairline bg-white">
            <table className="min-w-full text-left text-sm">
              ...
              <thead className="bg-paper text-xs font-mono uppercase text-ink-2">
                <tr>
              ...
              <tbody className="divide-y divide-hairline">
                {visibleEvents.map((event) => (
                  <tr key={`${event.id}-row`}>
                    <td className="px-3 py-2 font-mono text-ink-2">{formatLedgerDate(event.timestamp)}</td>
                    ...
                    <td className="px-3 py-2 font-mono text-ink-2">{event.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
